...
import { handleOpenAiError, isOpenAiError } from '../errors/openAiError';
...
export class GlobalExceptionFilter<T> implements ExceptionFilter {
  ...
  constructor(private readonly httpAdapterHost: HttpAdapterHost) {}
    ...
    // 🔟➕  OpenAI API Error (quota / auth / provider errors)
    else if (isOpenAiError(err)) {
      ({ statusCode, message, errorSources } = handleOpenAiError(err));
    }
